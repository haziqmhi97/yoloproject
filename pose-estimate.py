...
@torch.no_grad()
def run(poseweights="yolov7-w6-pose.pt",source="output_video_cropped.mp4",device='cpu',view_img=False,
    ...
    if (cap.isOpened() == False):   #check if videocapture not opened
        ...
        while(cap.isOpened): #loop until cap opened or video not complete
            ...
            if ret: #if success is true, means frame exist
                ...
                print(len(output),len(output[0]), len(output_data),len(output_data[0][0]))
                print(output,"\n@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n",output_data)
                ...
                for i, pose in enumerate(output_data):  # detections per image
                    ...
                    if len(output_data):  #check if no pose
                        for c in pose[:, 5]: # Print results .unique()
                            # print("++++", c)
                ...
                cv2.imwrite(f'image/Frame[{frame_count}].jpg', im0) #111
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default='output_video_cropped.mp4', help='video/0 for webcam') #video source
